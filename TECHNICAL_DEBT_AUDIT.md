...
**Naturaleza:** diagnóstico estático inicial, seguido por dos olas de remediación autorizadas. Las evidencias de los hallazgos describen el estado observado antes de esa remediación; la tabla de estado vigente indica qué cambió.
...
- **Descripción:** el repositorio contiene `fetch` en 46 archivos, pero guards de timeout (`AbortController`/`AbortSignal.timeout`) solo aparecen en 22 archivos; no es equivalencia uno-a-uno, pero confirma adopción no uniforme. OneDrive renueva tokens, crea carpetas, lista y descarga sin `signal`; la descarga se convierte completa a `Buffer`. El encadenamiento de materiales invoca otra función sin timeout ni autenticación. En contraste, curación y clientes modernos sí definen timeouts/retries.
- **Evidencia:** OneDrive `apps/web/src/domains/production/cloud-storage/microsoft-graph.service.ts:42-64,85-105,203-227,230-265`; trigger de materiales `apps/web/netlify/functions/shared/materials-generation-runtime.ts:95-110`; contraste con timeout en `apps/web/netlify/functions/shared/curation-runtime.ts:19-67`. Conteo estático reproducible sobre `apps/web/src` y `apps/web/netlify/functions`.
...
- **Criterios de aceptación:** toda salida de red tiene deadline explícito; tests simulan 429, 5xx, conexión colgada y respuesta grande; no se reintentan mutaciones sin idempotency key; concurrencia máxima y memoria quedan acotadas; métricas muestran latencia, retry y error por proveedor.
...
- **Evidencia:** `apps/web/src/lib/server/desktop-worker-control-plane.ts:585-607,665,770,1121-1147,1379-1383,1611-1643,2399-2445,2786-2937,3131`; `apps/web/src/domains/materials/hooks/useProductionAssetState.ts:282-307,375-377,493-494,763,1011-1061`; superficie de props `apps/web/src/domains/materials/components/ProductionAssetHeader.tsx:24-29`.
