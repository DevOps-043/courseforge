...
import { supportedAudioProfileIds } from "./worker-capabilities";
...
const supportedProfiles = supportedAudioProfileIds();
...
async function run() {
  while (!stopping) {
    try {
      const result = await processAudioProcessingBatch(supabase, supportedProfiles, batchSize);
