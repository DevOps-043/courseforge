...
function recoveredVideoFixture() {
  ...
  const video = normalizeVideoDurationContent({
    title: "Demostración", script: { sections: Array.from({ length: 7 }, (_, index) => ({
      section_number: index + 1, section_type: "demonstration", narration_text: "contenido ".repeat(90).trim(),
...
test("global resume selects only missing or invalid demo and preserves successful components", () => {
  ...
  // A cross-component or manually requested correction retains the full plan as fallback.
  assert.deepEqual(selectMaterialRetryTypes(input, saved), ["READING", "VIDEO_DEMO"]);
