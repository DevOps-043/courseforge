...
export function selectMaterialRetryTypes(input: MaterialsGenerationInput, saved: MaterialComponentRecord[]): string[] {
  ...
  // A complete lesson with a cross-component/manual issue needs the original plan.
  return retry.length ? retry : input.lesson.components.map((component) => component.type);
}
