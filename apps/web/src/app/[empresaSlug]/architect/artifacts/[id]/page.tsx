...
export default async function TenantArchitectArtifactPage({
  params,
}: {
    ...
    return (
      <div className="p-8 pb-32 max-w-7xl mx-auto">
        <div className="bg-red-50 border border-red-200 text-red-700 px-6 py-4 rounded-xl shadow-sm text-center">
          <h2 className="text-lg font-semibold mb-2">ID de Artefacto Invalido</h2>
          <p>El identificador proporcionado no tiene un formato valido.</p>
        </div>
      </div>
    );
  }
  ...
  const detail = await loadArtifactDetailPageData({
    ...
    fallbackRole: "ARQUITECTO",
    fallbackName: "Arquitecto",
  });

  if (!detail) {
    redirect(`${basePath}/artifacts`);
  }
