...
export default function ArtifactClientView({
  ...
  useEffect(() => {
    setEditedContent({
      ...
      descripcion: {
        texto:
          artifact.descripcion?.texto || artifact.descripcion?.resumen || "",
        publico_objetivo: artifact.descripcion?.publico_objetivo || "",
        beneficios: artifact.descripcion?.beneficios || "",
      },
    });
  ...
  const currentStatusStyle =
    statusColors[artifact.state] || statusColors.GENERATING;
                    ...
                    {reviewState === "pending" && (
                      <>
                        <button
                          onClick={async () => {
                            try {
                              const res = await updateArtifactStatusAction(
                                artifact.id,
                                "APPROVED",
                              );
                              if (res.success) {
...
function StepItem({ step, label, active, onClick, icon, disabled, done }: any) {
  return (
    ...
    >
      ...
      <span
        className={`text-xs font-medium uppercase ${active ? "text-blue-600 dark:text-[#1F5AF6]" : done ? "text-green-500 dark:text-[#00D4B3]" : "text-gray-400 dark:text-[#6C757D]"}`}
      >
        {label}
      </span>
    </button>
