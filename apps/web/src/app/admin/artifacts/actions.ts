 ...
 */
async function canReviewContent(userId: string) {
...
async function getAuthorizedArtifactAdmin(artifactId: string) {
...
async function getAuthorizedCurationRowAdmin(rowId: string) {
  const admin = getServiceRoleClient();
...
export async function generateArtifactAction(formData: {
  ...
  try {
    ...
    if (!triggerResponse.ok) {
      console.warn(
        `Background trigger might have failed: ${triggerResponse.status}`,
      );
    ...
    return {
      success: false,
      error: error.message || "Error initating generation",
    };
  }
}
...
export async function regenerateArtifactAction(
  ...
  try {
    ...
    const triggerResponse = await fetch(
      `${appUrl}/.netlify/functions/generate-artifact-background`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    if (!triggerResponse.ok) {
      console.warn(
        `Background regeneration trigger might have failed: ${triggerResponse.status}`,
      );
    }
...
export async function getInstructionalPlanSnapshotAction(artifactId: string) {
  ...
  if (error) {
    console.error("[InstructionalPlan] Snapshot error:", error);
    return { success: false, error: error.message };
  }
...
export async function getCurationSnapshotAction(artifactId: string) {
  ...
  if (curationError) {
    console.error("[Curation] Snapshot error:", curationError);
  ...
  if (curation?.id) {
    ...
    if (rowsError) {
      console.error("[Curation] Snapshot rows error:", rowsError);
...
export async function startCurationAction(
  ...
  try {
    ...
    if (existingCuration) {
      ...
      if (createError)
        throw new Error(
          `Failed to create curation record: ${createError.message}`,
        );
    ...
    console.log(
      "[Actions] Triggering Curation Background:",
    ...
    if (!triggerResponse.ok) {
      console.warn(
        `[Actions] Curation trigger failed: ${triggerResponse.status}`,
      );
    }
...
export async function importCurationJsonAction(
  ...
  try {
    ...
    try {
      ...
      return {
        success: false,
        error: "JSON inválido. Verifica el formato e intenta de nuevo.",
      };
    }
    ...
    if (
      ...
      return {
        success: false,
        error:
          'El JSON debe contener un array "sources" con al menos una fuente.',
      };
    }
    ...
    console.log(
      `[importCurationJson] Processing ${payload.sources.length} sources for artifact ${artifactId}`,
    );
    ...
    if (existingCuration) {
      ...
      if (deleteError) {
        console.warn(
          "[importCurationJson] Could not clear old GPT rows:",
          deleteError,
        );
      }
    } else {
      ...
      if (createError || !newCuration) {
        console.error(
          "[importCurationJson] Failed to create curation:",
          createError,
        );
        return {
          success: false,
          error: "Error creando registro de curaduría.",
        };
      }
      curationId = newCuration.id;
    }
...
export async function saveMaterialAssetsAction(
  ...
  const lessonForSync = component?.material_lessons as any;
  const materialsForSync = Array.isArray(lessonForSync?.materials)
    ? lessonForSync.materials[0]
    : lessonForSync?.materials;

  if (materialsForSync?.artifact_id) {
    await syncProductionStatusAction(materialsForSync.artifact_id);
  }
...
export async function syncProductionStatusAction(artifactId: string) {
  ...
  const produceable = components.filter(
    (c) => c.type.includes("VIDEO") || c.type === "DEMO_GUIDE",
  );
  ...
  const completed = produceable.filter(
    (c) => (c.assets as any)?.production_status === "COMPLETED",
  ).length;
...
export async function validateLessonAction(lessonId: string) {
  ...
  try {
    ...
    const missing = expectedTypes.filter(
      (type: string) => !generatedTypes.includes(type),
    );
    const errors: string[] = [];
    ...
    // Quiz validation
    const quizComponent = (components || []).find(
      (c: any) => c.type === "QUIZ",
    );
    ...
    const dod = {
      ...
      control5_quiz: quizComponent
        ? "PASS"
        : expectedTypes.includes("QUIZ")
          ? "FAIL"
          : "PASS",
      errors,
    };
...
export async function deleteArtifactAction(artifactId: string) {
  ...
  try {
    ...
    const { error: pubError } = await supabase
      ...
      .eq("artifact_id", artifactId);
    if (pubError)
      console.warn("Error deleting publication_requests:", pubError);
    ...
    const { error: planError } = await supabase
      ...
      .eq("artifact_id", artifactId);
    if (planError)
      console.warn("Error deleting instructional_plans:", planError);
    ...
    return {
      success: false,
      error: error.message || "Error eliminando dependencias.",
    };
  }
}
 ...
 */
export async function cascadeDeleteLessonAction(
  artifactId: string,
  lessonId: string,
  fromStep: number,
) {
  ...
  try {
    ...
    if (fromStep <= 2) {
      ...
      if (plan && plan.lesson_plans) {
        const filtered = (plan.lesson_plans as any[]).filter(
          (lp: any) => lp.lesson_id !== lessonId,
        );
        if (filtered.length !== (plan.lesson_plans as any[]).length) {
          await supabase
            .from("instructional_plans")
            .update({
              lesson_plans: filtered,
              updated_at: new Date().toISOString(),
            })
            .eq("id", plan.id);
    ...
    if (
      pubReq &&
      pubReq.selected_lessons &&
      Array.isArray(pubReq.selected_lessons)
    ) {
      const filtered = pubReq.selected_lessons.filter(
    ...
    console.log(
      `[cascadeDeleteLesson] Lesson ${lessonId} cascaded from step ${fromStep}:`,
      deletedFrom,
    );
 ...
 */
export async function cascadeDeleteModuleAction(
  artifactId: string,
  moduleId: string,
  fromStep: number,
) {
  ...
  try {
    ...
    if (!targetModule) {
      return {
        success: false,
        error: `Módulo ${moduleId} no encontrado en el temario`,
      };
    }

    const lessonIds = (targetModule.lessons || [])
      .map((l: any) => l.id)
      .filter(Boolean);
    ...
    for (const lessonId of lessonIds) {
      const result = await cascadeDeleteLessonAction(
        artifactId,
        lessonId,
        fromStep,
      );
    ...
    console.log(
      `[cascadeDeleteModule] Module ${moduleId} (${lessonIds.length} lessons) deleted from step ${fromStep}`,
    );
