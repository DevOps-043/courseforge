...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    logger.info("production.hyperframes.preset_application_applied", {
    ...
    logger.error("production.hyperframes.preset_application_apply_failed", error);
    return apiErrorResponse({ code: API_ERROR_CODE.internalError, headers: { "Cache-Control": "private, no-store" }, message: "No se pudo aplicar el preset.", requestId, retryable: true, status: 500 });
  }
}
