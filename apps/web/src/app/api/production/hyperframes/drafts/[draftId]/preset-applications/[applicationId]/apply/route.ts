...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    if (!precondition.ok) return NextResponse.json({ error: "Falta la versión base válida del documento." }, { status: 428 });
    ...
    return NextResponse.json({ error: "No se pudo aplicar el preset." }, { status: 500 });
  }
}
