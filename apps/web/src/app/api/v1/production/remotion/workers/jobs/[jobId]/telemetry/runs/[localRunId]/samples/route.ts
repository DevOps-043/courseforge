...
interface RouteContext {
  params: Promise<{ jobId: string; localRunId: string }>;
}
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const { jobId, localRunId } = await context.params;
    if (!isUuid(jobId)) return NextResponse.json({ error: "jobId must be a valid UUID" }, { status: 400 });

    const result = await auth.service.recordTelemetrySamples(auth.worker, jobId, localRunId, await parseJsonBody(request));
