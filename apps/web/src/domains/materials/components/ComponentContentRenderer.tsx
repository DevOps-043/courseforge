  ...
  return (
    <div className="space-y-4">
          ...
          <div key={i} className={`flex gap-3 ${scene.character === 'Usuario' ? 'flex-row-reverse' : ''}`}>
            ...
            <div
              className={`max-w-[85%] p-4 rounded-2xl text-sm shadow-sm ${
                scene.character === 'Usuario'
                  ? 'bg-[var(--engine-primary)] text-gray-100 rounded-tr-sm'
      ...
      {content.mission_brief && (
        <details className="mt-6 group border border-[var(--engine-info)]/20 rounded-xl overflow-hidden">
          <summary className="flex items-center gap-2 px-4 py-3 bg-[var(--engine-info)]/5 cursor-pointer select-none text-sm font-semibold text-[var(--engine-info)] hover:bg-[var(--engine-info)]/10 transition-colors">
          ...
          <div className="p-4 space-y-3 text-sm">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="px-2 py-0.5 bg-[var(--engine-info)]/10 text-[var(--engine-info)] rounded text-xs font-bold border border-[var(--engine-info)]/20">
              ...
              <ol className="space-y-1">
                {content.mission_brief.criterios_de_exito.map((c, i) => (
                  <li key={i} className="flex items-start gap-2 text-gray-700 dark:text-gray-200">
                    <span className="w-5 h-5 rounded-full bg-[var(--engine-accent)]/15 text-[var(--engine-accent)] text-xs font-bold flex items-center justify-center flex-shrink-0 mt-0.5">
                      {i + 1}
                    </span>
                    {c}
                  </li>
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        <div className="flex flex-wrap items-center gap-2">
          {content.title && <h4 className="font-semibold text-gray-900 dark:text-white text-lg">{content.title}</h4>}
          <span className="px-2 py-0.5 bg-[var(--engine-accent)]/10 text-[var(--engine-accent)] rounded text-[10px] font-bold border border-[var(--engine-accent)]/20">
            {content.runtimeType}
          </span>
          <span className="px-2 py-0.5 bg-[var(--engine-info)]/10 text-[var(--engine-info)] rounded text-[10px] font-bold border border-[var(--engine-info)]/20">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="p-4 bg-blue-50 dark:bg-[var(--engine-info)]/10 border border-blue-100 dark:border-[var(--engine-info)]/20 rounded-xl">
          <p className="text-[10px] font-bold uppercase tracking-wider text-[var(--engine-info)] mb-2">Opening message</p>
      ...
      <ul className="space-y-2">
        {items.map((item, index) => (
          <li key={`${title}-${index}`} className="text-sm text-gray-700 dark:text-gray-300 flex gap-2">
            <span className="w-1.5 h-1.5 rounded-full bg-[var(--engine-accent)] mt-2 flex-shrink-0" />
            <span>{item}</span>
          </li>
        ))}
      </ul>
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        {content.estimated_reading_time_min && (
          <div className="inline-flex items-center gap-2 text-xs font-medium text-[var(--engine-accent)] bg-[var(--engine-accent)]/10 px-2.5 py-1 rounded-full">
      ...
      {content.key_points && content.key_points.length > 0 && (
        <div className="p-5 bg-blue-50 dark:bg-[var(--engine-primary)]/30 border border-blue-100 dark:border-[var(--engine-primary)] rounded-xl shadow-sm">
          <p className="text-sm font-bold text-gray-900 dark:text-white mb-3 flex items-center gap-2">
            <CheckCircle className="w-4 h-4 text-[var(--engine-accent)]" />
            Puntos Clave
          </p>
          <ul className="space-y-3">
            {content.key_points.map((point, i) => (
              <li key={i} className="text-sm text-gray-700 dark:text-gray-300 flex items-start gap-3">
                <span className="w-1.5 h-1.5 rounded-full bg-[var(--engine-accent)] mt-2 flex-shrink-0 shadow-sm" />
      ...
      {content.reflection_question && (
        <div className="p-5 bg-gradient-to-r from-gray-50 to-white dark:from-[var(--engine-primary)] dark:to-[var(--engine-surface-hover)] border-l-4 border-[var(--engine-accent)] rounded-r-xl shadow-sm">
          <p className="text-xs font-bold uppercase tracking-wider text-[var(--engine-accent)] mb-2">Pregunta de Reflexión</p>
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between pb-4 border-b border-gray-100 dark:border-white/5">
        ...
        <div className="px-3 py-1 bg-gray-100 dark:bg-[var(--engine-surface-hover)] rounded-full border border-gray-200 dark:border-white/10 text-xs text-gray-500 dark:text-gray-400 font-medium">
        ...
        {content.items?.map((item, i) => (
          <div key={i} className="p-6 bg-white dark:bg-[#131820] border border-gray-200 dark:border-white/5 rounded-2xl shadow-sm transition-all hover:shadow-md hover:border-gray-300 dark:hover:border-white/10">
            <div className="flex gap-4 mb-6">
              <span className="flex-shrink-0 w-8 h-8 bg-[var(--engine-accent)]/10 text-[var(--engine-accent)] rounded-lg flex items-center justify-center text-sm font-bold border border-[var(--engine-accent)]/20">
                {i + 1}
              </span>
                  ...
                  return (
                    <div
                      ...
                      className={`p-3.5 rounded-xl border flex items-center gap-3 transition-colors ${
                        isCorrect
                          ? 'bg-emerald-50 dark:bg-[var(--engine-accent)]/10 border-emerald-200 dark:border-[var(--engine-accent)]/50'
                          : 'bg-gray-50 dark:bg-[var(--engine-canvas)] border-gray-100 dark:border-white/5 text-gray-600 dark:text-gray-400'
                      }`}
                    >
                      <div
                        className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${
                          isCorrect ? 'border-emerald-500 dark:border-[var(--engine-accent)] bg-emerald-500 dark:bg-[var(--engine-accent)]' : 'border-gray-300 dark:border-gray-600'
                        }`}
                      >
                        {isCorrect && <CheckCircle className="w-3.5 h-3.5 text-white dark:text-[var(--engine-primary)]" />}
                      </div>
...
function DemoGuideViewer({ content }: { content: DemoGuideContent }) {
  return (
    <div className="space-y-6">
      <div className="border-l-4 border-[var(--engine-accent)] pl-4">
      ...
      {content.prerequisites && content.prerequisites.length > 0 && (
        <div className="p-5 bg-gray-50 dark:bg-[var(--engine-surface-hover)] border border-gray-200 dark:border-white/5 rounded-xl">
      ...
      <div className="space-y-4">
        {content.steps?.map((step, i) => (
          <div key={i} className="group relative flex gap-4 p-5 bg-white dark:bg-[#131820] border border-gray-200 dark:border-white/5 rounded-xl transition-all hover:border-gray-300 dark:hover:border-white/10 shadow-sm">
            <div className="flex-shrink-0 w-8 h-8 flex items-center justify-center bg-[var(--engine-primary)] text-white rounded-lg border border-white/5 text-sm font-bold shadow-md">
              {step.step_number}
            </div>
            <div className="flex-1">
              <p className="text-sm text-gray-900 dark:text-gray-200 font-medium mb-2">{step.instruction}</p>
              {step.tip && (
                <p className="text-xs text-emerald-600 dark:text-[var(--engine-accent)] flex items-center gap-1.5 mt-2 bg-emerald-50 dark:bg-[var(--engine-accent)]/5 p-2 rounded-lg w-fit">
      ...
      {content.summary && (
        <div className="p-5 bg-blue-50 dark:bg-[var(--engine-primary)]/30 border border-blue-100 dark:border-[var(--engine-accent)]/30 rounded-xl relative overflow-hidden">
          <div className="absolute top-0 left-0 w-1 h-full bg-[var(--engine-accent)]" />
          <p className="text-sm font-bold text-[var(--engine-accent)] mb-1">Resumen</p>
...
function ExerciseViewer({ content }: { content: ExerciseContent }) {
  return (
    <div className="space-y-6">
      ...
      {content.instructions && (
        <div className="p-6 bg-slate-50 dark:bg-[var(--engine-primary)]/20 rounded-xl border border-slate-200 dark:border-white/5 shadow-sm">
          <p className="text-sm font-bold text-slate-700 dark:text-white mb-3 flex items-center gap-2">
            <ListOrdered className="w-4 h-4 text-[var(--engine-accent)]" />
            Instrucciones
          </p>
      ...
      {content.expected_outcome && (
        <div className="p-6 bg-emerald-50 dark:bg-[var(--engine-accent)]/5 rounded-xl border border-emerald-100 dark:border-[var(--engine-accent)]/10">
          <p className="text-sm font-bold text-emerald-700 dark:text-[var(--engine-accent)] mb-3 flex items-center gap-2">
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {content.duration_estimate_minutes && (
          <div className="flex items-center gap-2 text-xs font-bold text-[var(--engine-accent)] px-3 py-1.5 bg-[var(--engine-accent)]/10 rounded-full border border-[var(--engine-accent)]/20">
            <Clock className="w-3.5 h-3.5" />
          ...
          {content.script.sections.map((section, i) => (
            <div key={i} className="p-5 bg-white dark:bg-[#131820] border border-gray-200 dark:border-white/5 rounded-xl text-sm hover:border-gray-300 dark:hover:border-white/10 transition-colors shadow-sm">
              <div className="flex justify-between items-center mb-3">
                <span className="text-[10px] px-2 py-1 bg-[var(--engine-info)]/10 text-[var(--engine-info)] rounded font-bold border border-[var(--engine-info)]/20 uppercase tracking-wide">
      ...
      {content.storyboard && content.storyboard.length > 0 && (
        <details className="mt-4 group">
          <summary className="text-sm font-semibold cursor-pointer text-gray-600 dark:text-gray-300 hover:text-[var(--engine-accent)] dark:hover:text-[var(--engine-accent)] flex items-center gap-2 select-none">
            <ChevronRight className="w-4 h-4 transition-transform group-open:rotate-90" />
            Ver Storyboard ({content.storyboard.length} tomas)
          </summary>
          <div className="mt-4 overflow-x-auto rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[var(--engine-surface-solid)]">
