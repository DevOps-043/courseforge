...
export function LayoutOverrideDraftPanel({
  ...
  const displayX = selectedBox.x;
  const displayY = selectedBox.y;
  const displayWidth = selectedBox.width;
  const displayHeight = selectedBox.height;
  const isMoveMode = editMode === "move";
  const isCropMode = editMode === "crop";
      ...
      <div className="mb-4 flex flex-col gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
            {isMoveMode
              ? "Ajusta posicion, tamano, orden y alineacion con grid."
              : "Recorta los bordes visibles de la capa seleccionada."}
          </p>
        </div>
