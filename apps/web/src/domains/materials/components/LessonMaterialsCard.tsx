...
export function LessonMaterialsCard({ lesson, onIterationStart, onValidateLesson, onRegenerateLesson, onMarkForFix, className = '' }: LessonMaterialsCardProps) {
    ...
    return (
        <div className={`border rounded-2xl overflow-hidden transition-all duration-300 shadow-lg shadow-black/5 dark:shadow-black/20
            bg-white dark:bg-[var(--engine-surface-hover)] border-gray-200 dark:border-white/5
            hover:border-gray-300 dark:hover:border-[var(--engine-accent)]/30 ${className}`}>
            {/* Header */}
            <button
            ...
            >
                <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-full flex items-center justify-center
                        bg-gray-100 dark:bg-[var(--engine-primary)] border border-gray-200 dark:border-white/5
                        text-gray-600 dark:text-[var(--engine-accent)]">
                ...
                <div className="flex items-center gap-4">
                    <span className={`text-xs px-3 py-1 rounded-full border ${lesson.state === 'APPROVABLE' ? 'bg-[var(--engine-accent)]/10 border-[var(--engine-accent)]/20 text-[var(--engine-accent)]' :
            ...
            {/* Expanded Content */}
            {expanded && (
                <div className="border-t border-gray-100 dark:border-white/5 bg-gray-50/50 dark:bg-[var(--engine-canvas)]/50 p-6 space-y-6">
                    {/* OA */}
                    <div className="p-4 rounded-xl relative pl-10
                        bg-white dark:bg-[var(--engine-primary)]/20 border border-gray-200 dark:border-[var(--engine-primary)]/30">
                        <span className="absolute top-4 left-4 text-4xl leading-none opacity-30
                            text-blue-500 dark:text-[var(--engine-accent)]">"</span>
                        <p className="text-xs font-bold mb-1 uppercase tracking-wider
                            text-blue-600 dark:text-[var(--engine-accent)]">Objetivo de Aprendizaje</p>
                                ...
                                return (
                                    <div
                                        ...
                                        className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${generated
                                            ? 'bg-green-50 dark:bg-[var(--engine-accent)]/10 border-green-200 dark:border-[var(--engine-accent)]/30 text-green-700 dark:text-[var(--engine-accent)]'
                                            : 'bg-white dark:bg-white/5 border-gray-200 dark:border-white/5 text-gray-400 dark:text-gray-500'
                                            }`}
                                    >
                                        <div className={`w-1.5 h-1.5 rounded-full ${generated ? 'bg-green-500 dark:bg-[var(--engine-accent)]' : 'bg-gray-300 dark:bg-gray-600'}`} />
                                        {comp.replace(/_/g, ' ')}
                                    </div>
                                );
                            })}
                    ...
                    {loadingComponents ? (
                        ...
                        <div className="pt-2">
                            <button
                                ...
                                className="w-full group relative overflow-hidden rounded-xl bg-gradient-to-br from-[var(--engine-primary)] to-[var(--engine-surface-hover)] p-6 border border-white/10 hover:border-[var(--engine-accent)]/50 transition-all duration-300"
                            >
                                <div className="absolute inset-0 bg-[url('/grid.svg')] opacity-10" />
                                <div className="absolute inset-0 bg-gradient-to-r from-[var(--engine-accent)]/0 via-[var(--engine-accent)]/5 to-[var(--engine-accent)]/0 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000" />
                                ...
                                <div className="relative flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        <div className="w-12 h-12 rounded-lg flex items-center justify-center transition-transform group-hover:scale-110
                                            bg-white/20 dark:bg-[var(--engine-accent)]/10">
                                            <Sparkles className="w-6 h-6 text-white dark:text-[var(--engine-accent)]" />
                                        </div>
                                        <div className="text-left">
                                    ...
                                    <div className="w-10 h-10 rounded-full bg-white/20 dark:bg-white/5 flex items-center justify-center group-hover:bg-white group-hover:text-blue-600 dark:group-hover:bg-[var(--engine-accent)] dark:group-hover:text-white transition-colors">
