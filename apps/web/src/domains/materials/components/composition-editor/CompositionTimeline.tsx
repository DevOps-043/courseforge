...
export function CompositionTimeline({ assetLabels, currentTime, document, onClearSelection, onDurationChange, onMove, onSeek, onSelect, onTrackUpdate, onTrim, saving, selectedHfId, snapEnabled = true, trimMode = false }: CompositionTimelineProps) {
  ...
  const updateGesture = (event: PointerEvent<HTMLButtonElement>) => {
    ...
    if (gesture.kind === "move") {
      const startSeconds = quantizeTimelineSeconds(Math.max(0, Math.min(maxDuration - gesture.clip.durationSeconds, gesture.clip.startSeconds + deltaSeconds)), snapEnabled);
    ...
    if (gesture.kind === "trim-end") {
      ...
      const durationSeconds = quantizeTimelineSeconds(Math.max(0.05, Math.min(
    ...
    const startSeconds = quantizeTimelineSeconds(Math.max(earliestStart, Math.min(originalEnd - 0.05, gesture.clip.startSeconds + deltaSeconds)), snapEnabled);
    setGesture((current) => current ? {
      ...current,
      durationSeconds: quantizeTimelineSeconds(originalEnd - startSeconds, snapEnabled),
      sourceOffsetSeconds: Math.max(0, quantizeTimelineSeconds((gesture.clip.sourceOffsetSeconds || 0) + startSeconds - gesture.clip.startSeconds, snapEnabled)),
      startSeconds,
    } : current);
  };
...
function quantizeTimelineSeconds(value: number, snapEnabled: boolean) {
  return snapEnabled ? Math.round(value * 20) / 20 : Math.round(value * 1_000) / 1_000;
}
