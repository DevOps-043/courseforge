...
const PLAN_TERMINAL_STATES = new Set([
...
const PremiumSelect = ({
  ...
  const selectedLabel =
    options.find((o: any) => o.value === value)?.label || value || placeholder;
...
export function InstructionalPlanGenerationContainer({
  ...
  const fetchPlan = async (silent = false) => {
    ...
    try {
      ...
      if (data) {
        const hasLessons =
          Array.isArray(data.lesson_plans) && data.lesson_plans.length > 0;
        const hasValidation = Boolean(data.validation);
        const planReachedTerminalState = PLAN_TERMINAL_STATES.has(data.state);
        const shouldRefreshParent =
          silent &&
          ((lastKnownPlanStateRef.current !== data.state &&
        ...
        if (data.state === "STEP_FAILED") {
          setIsGenerating(false);
          toast.error(
            "La generación falló. Por favor, intenta de nuevo o revisa los logs.",
          );
        }
  ...
  const handleGenerate = async () => {
    ...
    lastKnownValidationRef.current = false;
    lastKnownPlanStateRef.current = "STEP_PROCESSING";
    setExistingPlan((current: any) =>
      current
        ? { ...current, state: "STEP_PROCESSING", validation: null }
        : current,
    );
    try {
      const { generateInstructionalPlanAction } =
        await import("../../../app/admin/artifacts/actions");
      ...
      if (!result.success) {
        ...
        toast.info(
          "Generación iniciada. Esto puede tomar entre 30 a 60 segundos.",
        );
  ...
  const handleValidate = async () => {
    ...
    lastKnownValidationRef.current = false;
    setExistingPlan((current: any) =>
      current ? { ...current, validation: null } : current,
    );
    try {
      const { validateInstructionalPlanAction } =
        await import("../../../app/admin/artifacts/actions");
  ...
  const groupedModules =
    existingPlan?.lesson_plans?.reduce((acc: any, lesson: any) => {
  ...
  const sortedModules = Object.values(groupedModules)
      ...
      <div className="max-w-5xl mx-auto space-y-8 pb-20 animate-in fade-in duration-500">
        <div className="flex justify-between items-center border-b border-gray-200 dark:border-gray-800 pb-6">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 text-sm mt-1 ml-12">
              {existingPlan.lesson_plans.length} lecciones planificadas •
              Iteración {existingPlan.iteration_count || 1}/5
            </p>
          </div>
                      ...
                      >
                        <div className="flex gap-4 flex-1">
                          <div className="mt-1 flex flex-col items-center gap-2">
                            ...
                            <div
                              className={`h-full w-0.5 rounded-full ${expandedLessonId === lesson.lesson_id ? "bg-[#00D4B3]/20" : "bg-transparent"}`}
                            />
                          </div>

                          <div className="space-y-1 flex-1">
                            <h4
                              className={`font-semibold text-base transition-colors ${expandedLessonId === lesson.lesson_id ? "text-[#00D4B3]" : "text-gray-900 dark:text-gray-200 group-hover:text-[#00D4B3] dark:group-hover:text-white"}`}
                            >
                              {lesson.lesson_title}
                            </h4>
                            ...
                            {!isEditing && (
                              <div className="flex flex-wrap gap-2 pt-1">
                                ...
                                {lesson.components.map(
                                  (comp: any, cIdx: number) => {
                                    const badge = getComponentBadge(comp.type);
                                    return (
                                      <span
                                        key={cIdx}
                                        className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-[10px] font-medium tracking-wide border uppercase ${badge.color}`}
                                      >
                                        {badge.icon}
                                        {badge.label}
                                      </span>
                          ...
                          {isEditing ? (
                            ...
                            <div className="space-y-6 bg-white dark:bg-[#0A0E12] p-4 rounded-2xl border border-gray-200 dark:border-gray-800/60 shadow-inner">
                              ...
                              <div className="space-y-2">
                                ...
                                <PremiumTextarea
                                  value={
                                    displayLesson.learning_objective ||
                                    displayLesson.oa_text ||
                                    ""
                                  }
                                  onChange={(e: any) =>
                                    setEditedLesson({
                              ...
                              <div className="space-y-2">
                                ...
                                <PremiumInput
                                  value={
                                    displayLesson.measurable_criteria || ""
                                  }
                                  onChange={(e: any) =>
                                    setEditedLesson({
                                      ...displayLesson,
                              ...
                              <div className="space-y-4">
                                ...
                                {displayLesson.components.map(
                                  (comp: Component, cIdx: number) => (
                                    <div
                                      key={cIdx}
                                      className="bg-gray-50 dark:bg-[#151A21]/50 border border-gray-200 dark:border-gray-700/50 rounded-xl p-4 space-y-3 relative group/edit-card hover:border-gray-300 dark:hover:border-gray-600 transition-colors"
                                    >
                            ...
                            <>
                              ...
                              <div className="space-y-3">
                                ...
                                <div className="grid gap-3">
                                  {lesson.components.map(
                                    (comp: any, idx: number) => {
                                      const badge = getComponentBadge(
                                        comp.type,
                                      );
                                      return (
                                        <div
                                          key={idx}
                                          className="bg-white dark:bg-[#161b22] p-3 rounded-lg border border-gray-200 dark:border-gray-800 flex gap-3 group/card hover:border-gray-300 dark:hover:border-gray-700 transition-colors"
                                        >
                                          <div
                                            className={`mt-0.5 p-1.5 rounded h-fit ${badge.color.split(" ")[1]} ${badge.color.split(" ")[0]}`}
                                          >
                                            {badge.icon}
                                          </div>
        ...
        {existingPlan.validation && (
          <div className="pt-8 border-t border-gray-200 dark:border-gray-800">
            <InstructionalPlanValidationResult
            ...
            {canReview &&
              ...
              existingPlan.state !== "STEP_REJECTED" && (
                <>
                  ...
                  <button
                    onClick={async () => {
                      const { updateInstructionalPlanStatusAction } =
                        await import("../../../app/admin/artifacts/actions");
                      await updateInstructionalPlanStatusAction(
                        artifactId,
                        "STEP_REJECTED",
              ...
              <div className="w-full flex gap-4">
                ...
                <button
                  onClick={async () => {
                    if (
                      !confirm(
                        "¿Estás seguro de que quieres regenerar? Esto eliminará el plan actual.",
                      )
                    )
                      return;
                    try {
                      const { deleteInstructionalPlanAction } =
                        await import("../../../app/admin/artifacts/actions");
    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-500 relative">
      ...
      <div className="bg-[#151A21] border border-[#6C757D]/10 rounded-2xl p-6 shadow-xl shadow-black/20 transition-all duration-300">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center gap-2">
            <span
              className={`text-xs font-medium transition-colors ${useCustomPrompt ? "text-[#00D4B3]" : "text-[#6C757D]"}`}
            >
              {useCustomPrompt
            ...
            >
              <div
                className={`absolute top-0.5 w-3.5 h-3.5 rounded-full transition-all duration-300 shadow-sm ${useCustomPrompt ? "left-5 bg-[#00D4B3]" : "left-0.5 bg-[#6C757D]"}`}
              />
            </button>
          </div>
        </div>
          ...
          {useCustomPrompt ? (
            ...
            <div className="bg-[#0F1419] border border-[#6C757D]/10 rounded-xl p-6 flex flex-col gap-4 group hover:border-[#00D4B3]/20 transition-colors cursor-default animate-in fade-in duration-300 relative overflow-hidden">
              ...
              <p className="text-[#94A3B8] text-sm leading-relaxed relative z-10">
                Prompt optimizado para generar lecciones detalladas alineadas
                con el temario aprobado. Incluye la definición de objetivos de
                aprendizaje, criterios de éxito medibles y 4 componentes
                obligatorios por lección: Diálogo, Lectura, Quiz y Video, con
          ...
          <>
            ...
            <span>
              ...
              {existingPlan?.lesson_plans?.length > 0
                ? `(${existingPlan.lesson_plans.length})`
                : ""}
            </span>
          </>
        ) : (
