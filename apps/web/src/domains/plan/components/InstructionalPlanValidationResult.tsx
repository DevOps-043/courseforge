    ...
    return (
        <div className="space-y-2">
            <div className="flex justify-between text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">
                <span>{label}</span>
                <span className={value >= 90 ? "text-[var(--engine-accent)]" : "text-gray-900 dark:text-white"}>{value}%</span>
            </div>
            <div className="h-1.5 w-full bg-gray-200 dark:bg-gray-800 rounded-full overflow-hidden">
                <div 
                    className={`h-full rounded-full transition-all duration-1000 ease-out ${value >= 80 ? 'bg-[var(--engine-accent)]' : value >= 60 ? 'bg-yellow-500' : 'bg-red-500'}`}
                    style={{ width: `${value}%` }}
                />
            </div>
        </div>
    ...
    return (
        <div className="space-y-4 animate-in fade-in slide-in-from-top-4 duration-700">
            <div className="flex items-center gap-2 mb-2">
                <div className="p-1.5 bg-[var(--engine-accent)]/20 rounded-full">
                    <ShieldCheck size={16} className="text-[var(--engine-accent)]" />
                </div>
            ...
            <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-[#0A0E12] overflow-hidden shadow-2xl shadow-black/5 dark:shadow-black/50">
                ...
                <div className="p-6 border-b border-gray-200 dark:border-gray-800 flex flex-col md:flex-row justify-between items-start gap-6 relative overflow-hidden">
                    {/* Background Glow */}
                    <div className={`absolute top-0 right-0 w-64 h-64 ${isApproved ? 'bg-[var(--engine-accent)]/5' : 'bg-orange-500/5'} blur-[100px] pointer-events-none`} />

                    <div className="flex gap-5 z-10 w-full">
                        <div className={`p-4 rounded-2xl h-fit shrink-0 ${isApproved ? 'bg-[var(--engine-accent)]/10 text-[var(--engine-accent)]' : 'bg-orange-500/10 text-orange-500'}`}>
                        ...
                        <div className="space-y-3 w-full">
                            <div className="flex justify-between items-start w-full">
                                ...
                                <div className="text-right md:hidden">
                                     <div className={`text-4xl font-bold tracking-tighter ${isApproved ? 'text-[var(--engine-accent)]' : 'text-orange-500'}`}>
                                        {score_general}%
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="text-right hidden md:block shrink-0 z-10 pl-6 border-l border-gray-200 dark:border-gray-800/50">
                         <div className={`text-5xl font-bold tracking-tighter ${isApproved ? 'text-[var(--engine-accent)]' : 'text-orange-500'}`}>
                            {score_general}%
                        </div>
                ...
                {/* Metrics Grid */}
                <div className="p-8 bg-gray-50 dark:bg-[var(--engine-canvas)]/30">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-200 dark:divide-gray-800 border-t border-gray-200 dark:border-gray-800">
                    {/* Fortalezas */}
                    <div className="p-6 space-y-4 bg-[var(--engine-accent)]/5 dark:bg-[var(--engine-accent)]/[0.02]">
                        <div className="flex items-center gap-2 text-[var(--engine-accent)] font-semibold text-sm uppercase tracking-wide mb-2">
                            <Sparkles size={16} />
                            Fortalezas
                        </div>
                        <ul className="space-y-3">
                            {fortalezas.length > 0 ? fortalezas.map((item: string, idx: number) => (
                                <li key={idx} className="flex gap-3 text-sm text-gray-700 dark:text-gray-400">
                                    <CheckCircle2 size={14} className="text-[var(--engine-accent)] mt-0.5 shrink-0" />
                                    <span>{item}</span>
                                </li>
