...
export const PremiumInput = ({ className, value, ...props }: PremiumInputProps) => (
  <input
    className={`w-full px-4 py-2.5 rounded-xl bg-white border border-gray-200
              text-gray-900 placeholder-gray-400 focus:outline-none focus:border-[var(--engine-accent)]/50 focus:bg-white
  ...
  return (
    <div className={`relative ${className}`} ref={containerRef}>
      <button
        ...
        className="w-full px-3 py-2 rounded-lg bg-white border border-gray-200 flex items-center justify-between gap-2 text-xs font-medium text-gray-900 hover:border-[var(--engine-accent)]/50 transition-all dark:bg-white/5 dark:border-white/10 dark:text-white"
      >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute top-full left-0 right-0 mt-1 rounded-xl border border-gray-200 bg-white shadow-2xl overflow-hidden z-[50] min-w-[150px] dark:border-white/10 dark:bg-[var(--engine-surface-hover)]"
          >
              ...
              {options.map((option) => (
                <button
                  ...
                  className={`w-full text-left px-4 py-2 text-xs hover:bg-gray-50 transition-colors flex items-center justify-between dark:hover:bg-white/5
                            ${option.value === value ? "text-[var(--engine-accent)] bg-[var(--engine-accent)]/10" : "text-gray-700 dark:text-gray-300"}
                    `}
                >
                  {option.label}
