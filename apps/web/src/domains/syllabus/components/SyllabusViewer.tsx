  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'SYLLABUS' && (
        <>
            {/* Resumen Header */}
            <div className="flex items-center gap-4 bg-[#151A21] border border-white/5 p-4 rounded-xl text-sm justify-between">
      ...
      {modules.map((module, index) => {
        ...
        return (
            <div 
            key={index} 
            className={`group bg-[#151A21] rounded-2xl border ${isEditing ? 'border-[#00D4B3] ring-1 ring-[#00D4B3]/30' : 'border-white/5'} overflow-hidden transition-all duration-300`}
            >
            {/* Header del Módulo */}
            <div className="w-full px-6 py-5 flex justify-between items-start">
                <div onClick={() => !isEditing && toggleModule(index)} className={`flex-1 ${!isEditing && 'cursor-pointer'}`}>
                    <div className="flex items-center gap-2 mb-1">
                        {isEditing ? (
                             <input 
                                className="bg-[#0F1419] border border-white/10 rounded-xl px-4 py-3 text-lg font-bold text-white w-full focus:outline-none focus:border-[#00D4B3] transition-all focus:bg-[#151A21] placeholder-gray-600 shadow-inner"
                ...
                <div className="flex items-center gap-3 pl-4">
                    {!isEditing && isEditable && (
                        <button 
                            ...
                            className="flex items-center gap-1.5 px-3 py-1.5 bg-white/5 hover:bg-white/10 rounded-lg text-gray-400 hover:text-[#00D4B3] transition-colors border border-white/5 hover:border-[#00D4B3]/30 text-xs font-medium"
                        >
                            ...
                            <div className="flex-grow min-w-0 space-y-2">
                                {isEditing ? (
                                    <div className="space-y-3">
                                        <input 
                                            className="w-full bg-[#0F1419] border border-white/10 rounded-xl px-4 py-3 text-base font-medium text-white focus:outline-none focus:border-[#00D4B3] transition-all focus:bg-[#151A21] placeholder-gray-600"
                                            value={lesson.title}
                                            onChange={(e) => updateLesson(index, lIndex, 'title', e.target.value)}
                                            placeholder="Título de la lección"
                                        />
                                        <textarea
                                            className="w-full bg-[#0F1419] border border-white/10 rounded-xl px-4 py-3 text-sm text-gray-300 focus:outline-none focus:border-[#00D4B3] min-h-[80px] transition-all focus:bg-[#151A21] resize-none placeholder-gray-600"
                                        ...
                                        />
                                        <div className="flex items-center gap-3 bg-[#151A21] rounded-xl px-4 py-2 border border-white/5 w-fit hover:border-white/10 transition-colors group/time">
                                            ...
                                            <div className="flex flex-col justify-center">
                                                ...
                                                <div className="flex items-baseline gap-1">
                                                     <input 
                                                        type="number"
                                                        className="bg-transparent border-none p-0 text-sm font-bold text-white w-12 focus:outline-none focus:ring-0 font-mono text-right appearance-none [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
