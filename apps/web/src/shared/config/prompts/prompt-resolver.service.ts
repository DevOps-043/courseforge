...
export async function resolvePrompts(
    ...
    // Determine which prompt codes we need
    ...
    // Fetch all matching prompts from DB in a single query
    const dbPrompts = await fetchPromptsFromDb(supabase, codesNeeded, organizationId);

    // Resolve system prompt
    const systemPrompt = dbPrompts.get(SYSTEM_PROMPT_CODE)
        ?? DEFAULT_PROMPTS[SYSTEM_PROMPT_CODE]
        ?? '';

    // Resolve component prompts
...
export function assemblePrompt(
    ...
    // 1. System prompt (rules, format, accessibility)
