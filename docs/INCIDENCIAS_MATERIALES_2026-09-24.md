...
| Regeneración global repite trabajo correcto | Selección de componentes faltantes o inválidos utilizando los componentes guardados y los contratos del plan. Si no quedan componentes defectuosos, reanudar revalida lo guardado sin llamar al proveedor ni reemplazar contenido. Los problemas transversales permanecen visibles en la validación completa; la regeneración completa requiere el flujo manual. |
...
## Integración con la política de duración (25 de septiembre)

Se conservan las pruebas de tomas breves y de margen editorial: el mínimo y máximo
reciben el 5 %, incluso cuando coinciden. El prompt global permite una toma en una
sección breve y mantiene la cadencia y el mínimo global de tomas.

Reanudar consulta los componentes guardados incluso en la primera iteración.
Una selección vacía pasa por validación completa y por `commit_material_generation`
con cero filas de componentes: conserva IDs, contenido y assets; solo actualiza
el estado de la ejecución vigente. Un fallo de validación mantiene `NEEDS_FIX` y
una ejecución sustituida no se reporta como exitosa. La regeneración manual sigue
solicitando contenido nuevo. No se cambió el esquema ni se añadieron dependencias.

Las regresiones verifican un guion de aproximadamente 7.000 caracteres, antes
rechazado por el objetivo de 7 minutos y ahora válido para el rango de 6–8 minutos,
sin requerir modelo y sin reescribir componentes, en iteraciones 1 y 2 y ante
una ejecución sustituida. Se mantienen pendientes las comprobaciones con
proveedores y base de datos reales después del despliegue.
