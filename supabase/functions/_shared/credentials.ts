...
export async function getOrganizationHyperframesApiKey(organizationId: string): Promise<string> {
  const { data, error } = await getAdminClient()
    .from("production_provider_credentials")
    .select("encrypted_secret")
    .eq("organization_id", organizationId)
    .eq("provider", "hyperframes_cloud")
    .eq("status", "ACTIVE")
    .maybeSingle();
  if (error) throw new Error(`Credential lookup failed: ${error.message}`);
  if (!data?.encrypted_secret) {
    throw new Error("No active HyperFrames credential for organization.");
  }
