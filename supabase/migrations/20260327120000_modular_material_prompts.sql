...
**Reglas críticas:**
- **storyboard** arrays deben tener timecodes coherentes y progresivos.
- El `narration_text` en CADA TOMA debe ser el guion EXACTO a locutar. NO hagas resúmenes.
- La suma de los `narration_text` del storyboard debe ser el script completo al 100%.
- **visual_content** debe describir EXACTAMENTE lo que se ve en pantalla (sin metáforas abstractas).$$,
...
**REGLA DE ORO PARA STORYBOARDS:**
El texto narrativo (`narration_text`) en el storyboard **DEBE SER EL GUIÓN EXACTO Y LITERAL** que se va a leer. NO hagas resúmenes. La sincronización entre guion y visuales debe ser perfecta y 1:1.
  ...
  "storyboard": [
    {
      ...
      "on_screen_text": "string (opcional)",
      "narration_text": "string (GUIÓN EXACTO Y LITERAL)",
      "operational_notes": "string (opcional)"
    }
  ],
  "parallel_exercise": {
