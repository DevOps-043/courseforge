-- Normalize active generation prompts so DIALOGUE materials target SofLIA
-- Learning's SOFLIA_DIALOGUE runtime instead of legacy scenes/lia_script.
--
-- Safe to run more than once. It creates/updates one active MATERIALS_DIALOGUE
-- v2.0.0 prompt for every scope that already had an active prompt:
-- global (organization_id is null) and organization-specific overrides.
-- Organization-specific overrides are important because prompt resolution checks
-- them before global defaults.

BEGIN;

CREATE TEMP TABLE tmp_materials_dialogue_prompt_scopes (
  organization_id uuid
) ON COMMIT DROP;

INSERT INTO tmp_materials_dialogue_prompt_scopes (organization_id)
...
Usa el OA, el nivel Bloom, el resumen del componente y las fuentes curadas. La actividad debe abrir espacio a razonamiento, ejemplos y transferencia profesional, no solo definiciones.$$,
  ...
  true,
  organization_id
FROM tmp_materials_dialogue_prompt_scopes
...
-- PostgreSQL unique constraints treat NULL organization_id as distinct, so the
-- ON CONFLICT path above does not deduplicate the global row. Keep only one
-- active global v2 row if repeated manual runs created more than one.
WITH ranked_global_prompts AS (
  SELECT
    id,
    row_number() OVER (ORDER BY updated_at DESC, created_at DESC, id DESC) AS rank
  FROM public.system_prompts
  WHERE code = 'MATERIALS_DIALOGUE'
    AND version = '2.0.0'
...
SET
  is_active = false,
  updated_at = now()
FROM ranked_global_prompts ranked
