-- Atomically append a native editor document and its audit record.
-- The expected hash is the optimistic concurrency guard for multiple browser tabs.
...
BEGIN
  ...
  SELECT document_hash, version
  INTO v_current_hash, v_current_version
  FROM public.video_composition_draft_documents
  WHERE draft_id = p_draft_id AND organization_id = p_organization_id
  ORDER BY version DESC
  LIMIT 1
  FOR UPDATE;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Draft document not found' USING ERRCODE = 'P0002';
  END IF;
